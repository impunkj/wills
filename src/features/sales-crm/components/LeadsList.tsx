  ...
  if (editModal.open && editModal.lead) {
    ...
    return (
      <LeadForm
        ...
        onSave={(data) => {
          if (editModal.lead) {
            void onEditLead?.(editModal.lead.id, data)
              ...
              onClick={() => {
                if (followUpModal.lead) {
                  const nextFollowUp: FollowUpInput = {
                    type: followUpType === 'Meeting' ? 'meeting' : followUpType === 'WhatsApp' ? 'quotation' : 'update',
                  ...
                  void onAddFollowUp?.(followUpModal.lead.id, nextFollowUp)
                }
