...
export function TeamManagement({
  ...
  const [activeTab, setActiveTab] = useState<TabKey>(view === 'lawyers-only' ? 'lawyers' : 'users')
  ...
  function handleSaveLawyer() {
    const payload = {
      ...
      specialization: lawyerForm.specialization
        ...
        .map((item) => item.trim())
        .filter(Boolean),
      barNumber: lawyerForm.barCouncilId.trim(),
    }
  ...
  function openEditEmployee(emp: Employee) {
    setEmployeeForm({ name: `${emp.firstName} ${emp.lastName}`, email: emp.email, phone: emp.mobile, department: emp.department, designation: emp.designation, status: emp.status })
    ...
    <div className="space-y-6 pb-8">
      ...
      {activeTab === 'users' && (
        <UsersTab
          ...
          onAdd={openAddUser}
          onEdit={(id: string) => { const u = users.find((x) => x.id === id); if (u) openEditUser(u) }}
          onToggleStatus={(id: string) => { const u = users.find((x) => x.id === id); if (u) setToggleUserStatusTarget(u) }}
        />
      )}
      {activeTab === 'lawyers' && (
        <LawyersTab
